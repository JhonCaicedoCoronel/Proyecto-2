...
public class PagoController implements Initializable {
  ...
  @FXML
    void ComprobarCodigo(ActionEvent event)throws DatosIncompletosExcpetion {
        for (Promocion p: PromocionesController.promociones){
           if(p.getCodigo().equals(txtcodigo.getText())){
           Codigo.setText("Descuento: "+String.valueOf(p.getDescuento())); 
           Total=Total-p.getDescuento();
           Descuento=p.getDescuento();
           PagoTotal.setText(String.valueOf(Total));
           if(p==PromocionesController.promociones.get(PromocionesController.promociones.size()-1)&p.getCodigo()!=txtcodigo.getText()){
               DatosIncompletosExcpetion dt=new DatosIncompletosExcpetion("Debe llenar los datos");
               dt.printStackTrace();
      }
    ...
    void Pagar(ActionEvent event) throws IOException {
        if(rb2.isSelected()==true){
            if(txtnumero.getText()==null||txtCVV.getText()==null|| txtcodigo.getText()!=null){
                try{
